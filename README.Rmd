...
## Current live operations (json files on netork folder)

The scripts prepare a set of json files saved in the network folder:
`r paste0(destination_dir,"/data")`

The **main scripts** are:
- `copy_to_production`: Main script executing all the other necessary ones. It's the one automated to be run every morning.
- `generate_json_files_nw/ao/states`: Retrieve the raw data, generate the json files and saves them locally in the repo's data folder.
