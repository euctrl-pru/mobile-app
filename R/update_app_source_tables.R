...
import_dataframe <- function(dfname) {
  ...
  df_base <- read_partitioned_parquet_duckdb(
    con = con,
  ...
  if (!exists) {
    duckdb::duckdb_register(con, "list_ao_reg", as.data.frame(list_ao))
  }
  ...
  if (!exists) {
    duckdb::duckdb_register(
      con,
      "list_airport_extended_reg",
      as.data.frame(list_airport_extended)
    )
  }
  ...
  if (!exists) {
    duckdb::duckdb_register(
      con,
      "list_iso_country_reg",
      as.data.frame(list_iso_country)
    )
  }
  ...
  if (!exists) {
    duckdb::duckdb_register(
      con,
      "list_icao_country_spain_reg",
      as.data.frame(list_icao_country_spain)
    )
  }
  ...
  if (!exists) {
    duckdb::duckdb_register(
      con,
      "list_marktet_segment_app_reg",
      as.data.frame(list_marktet_segment_app)
    )
  }
  ...
  if (dfname == "ao_st_des") {
    ...
    dfname == "ap_st_des" | dfname == "ap_ap_des" | dfname == "ap_ms"
  ) {
    df_app <- df_alldays %>%
      left_join(tbl(con, "list_airport_extended_reg"), by = c("BK_AP_ID")) %>%
      select(-EC_AP_CODE) %>%
    ...
    df_app <- df_alldays %>%
      ...
      left_join(
        tbl(con, "list_icao_country_spain_reg"),
        by = c("COUNTRY_CODE")
      ) %>%
...
stk_daily <- function(
  ...
  if (!rlang::quo_is_null(dly_quo)) {
    dly_name <- rlang::as_name(dly_quo)
  }
  ...
  list_icao_country_f <- list_icao_country %>%
    mutate(
      STK_ID = COUNTRY_CODE,
      STK_CODE = COUNTRY_CODE,
      STK_NAME = COUNTRY_NAME
    ) %>%
    select(STK_ID, STK_CODE, STK_NAME)

  list_nw_f <- data.frame(
    STK_ID = c("NW"),
    STK_CODE = c("NW"),
    STK_NAME = c("Total Network Management Area")
  )
  ...
  days_stk <- crossing(days_sequence, get(list_stk)) %>%
    ...
    mutate(
      SUMMER_FLAG = if_else(
        between(
          as.integer(format(FLIGHT_DATE, "%m%d")),
          summer_start,
          summer_end
        ),
        1,
  ...
  df_day_year <- days_stk %>%
    ...
    mutate(
      ...
      RWK_DLY_FLT = if_else(
        RWK_AVG_TFC_DLY == 0,
        0,
        RWK_AVG_DLY / RWK_AVG_TFC_DLY
      ),
      RWK_DLY_ERT_FLT = if_else(
        RWK_AVG_TFC_DLY == 0,
        0,
        RWK_AVG_DLY_ERT / RWK_AVG_TFC_DLY
      ),
      RWK_DLY_ARP_FLT = if_else(
        RWK_AVG_TFC_DLY == 0,
        0,
        RWK_AVG_DLY_ARP / RWK_AVG_TFC_DLY
      ),
      ...
      RWK_DLYED_PERC = if_else(
        RWK_AVG_TFC_DLY == 0,
        0,
        RWK_AVG_DLYED / RWK_AVG_TFC_DLY
      ),

      # delayed flights 15 min
      DAY_DLYED_15 = coalesce(DLYED_15, 0),
      RWK_AVG_DLYED_15 = rollsum(DAY_DLYED_15, 7, fill = NA, align = "right") /
        7,
      DAY_DLYED_15_PERC = if_else(
        DAY_TFC_DLY == 0,
        0,
        DAY_DLYED_15 / DAY_TFC_DLY
      ),
      RWK_DLYED_15_PERC = if_else(
        RWK_AVG_TFC_DLY == 0,
        0,
        RWK_AVG_DLYED_15 / RWK_AVG_TFC_DLY
      )
    ) %>%
    ...
    mutate(
      mutate(across(
        starts_with("RWK"),
        ~ replace(.x, FLIGHT_DATE > mydate, NA)
      ))
    ) %>%
    # y2d calculations
    ...
    mutate(
      ...
      S2D_AVG_TFC = if_else(
        cumsum(SUMMER_FLAG) == 0,
        0,
        S2D_TFC / cumsum(SUMMER_FLAG)
      ),
      S2D_TFC_DLY = cumsum(DAY_TFC_DLY * SUMMER_FLAG),
      S2D_AVG_TFC_DLY = if_else(
        cumsum(SUMMER_FLAG) == 0,
        0,
        S2D_TFC_DLY / cumsum(SUMMER_FLAG)
      ),
      ...
      S2D_AVG_DLY = if_else(
        cumsum(SUMMER_FLAG) == 0,
        0,
        S2D_DLY / cumsum(SUMMER_FLAG)
      ),
      S2D_AVG_DLY_ERT = if_else(
        cumsum(SUMMER_FLAG) == 0,
        0,
        S2D_DLY_ERT / cumsum(SUMMER_FLAG)
      ),
      S2D_AVG_DLY_ARP = if_else(
        cumsum(SUMMER_FLAG) == 0,
        0,
        S2D_DLY_ARP / cumsum(SUMMER_FLAG)
      ),
      ...
      Y2D_DLYED_15_PERC = if_else(
        Y2D_TFC_DLY == 0,
        0,
        Y2D_DLYED_15 / Y2D_TFC_DLY
      ),

      S2D_DLYED_15 = cumsum(DAY_DLYED_15 * SUMMER_FLAG),
      S2D_DLYED_15_PERC = if_else(
        S2D_TFC_DLY == 0,
        0,
        S2D_DLYED_15 / S2D_TFC_DLY
      )
    ) %>%
    ungroup()
  ...
  df_day <- df_day_year %>%
    mutate(
      ...
      FLIGHT_DATE_2019 = FLIGHT_DATE -
        days((YEAR - 2019) * 364 + floor((YEAR - 2019) / 4) * 7),
      FLIGHT_DATE_2020 = FLIGHT_DATE -
        days((YEAR - 2020) * 364 + floor((YEAR - 2020) / 4) * 7),
    ...
    left_join(
      select(
        ...
        YEAR,
        FLIGHT_DATE,
        starts_with(c("Y2D", "S2D"))
      ),
      by = c("STK_ID", "FLIGHT_DATE_PREV_YEAR_SD" = "FLIGHT_DATE"),
      suffix = c("", "_PREV_YEAR")
    ) %>%
    left_join(
      select(
      ...
      by = c("STK_ID", "FLIGHT_DATE_2019_SD" = "FLIGHT_DATE"),
      suffix = c("", "_2019")
    ) %>%
    left_join(
      select(
        ...
        YEAR,
        FLIGHT_DATE,
        starts_with(c("DAY", "RWK"))
      ),
      by = c("STK_ID", "FLIGHT_DATE_2019" = "FLIGHT_DATE"),
      suffix = c("", "_2019")
    ) %>%
    left_join(
      select(
      ...
      by = c("STK_ID", "FLIGHT_DATE_PREV_YEAR" = "FLIGHT_DATE"),
      suffix = c("", "_PREV_YEAR")
    ) %>%
    left_join(
      select(df_day_year, STK_ID, YEAR, FLIGHT_DATE, RWK_AVG_TFC),
      by = c("STK_ID", "FLIGHT_DATE_2020" = "FLIGHT_DATE"),
      suffix = c("", "_2020")
    ) %>%
    arrange(STK_ID, FLIGHT_DATE) %>%
    group_by(STK_ID) %>%
    mutate(
      ...
      # dif prev week
      ##traffic
      DAY_TFC_DIF_PREV_WEEK = coalesce(DAY_TFC, 0) -
        coalesce(DAY_TFC_PREV_WEEK, 0),
      DAY_TFC_DIF_PREV_WEEK_PERC = if_else(
        DAY_TFC_PREV_WEEK == 0,
        NA,
        DAY_TFC / DAY_TFC_PREV_WEEK
      ) -
        1,
      RWK_TFC_DIF_PREV_WEEK_PERC = if_else(
        RWK_AVG_TFC_PREV_WEEK == 0,
        NA,
        RWK_AVG_TFC / RWK_AVG_TFC_PREV_WEEK
      ) -
        1,
      ##delay
      DAY_DLY_DIF_PREV_WEEK_PERC = if_else(
        DAY_DLY_PREV_WEEK == 0,
        NA,
        DAY_DLY / DAY_DLY_PREV_WEEK
      ) -
        1,
      RWK_DLY_DIF_PREV_WEEK_PERC = if_else(
        RWK_AVG_DLY_PREV_WEEK == 0,
        NA,
        RWK_AVG_DLY / RWK_AVG_DLY_PREV_WEEK
      ) -
        1,
      ##delay/flight
      DAY_DLY_FLT_DIF_PREV_WEEK_PERC = if_else(
        DAY_DLY_FLT_PREV_WEEK == 0,
        NA,
        DAY_DLY_FLT / DAY_DLY_FLT_PREV_WEEK
      ) -
        1,
      RWK_DLY_FLT_DIF_PREV_WEEK_PERC = if_else(
        RWK_DLY_FLT_PREV_WEEK == 0,
        NA,
        RWK_DLY_FLT / RWK_DLY_FLT_PREV_WEEK
      ) -
        1,
      ##delayed flights
      ...
      ##delayed flights 15 min
      DAY_DLYED_15_PERC_DIF_PREV_WEEK = DAY_DLYED_15_PERC -
        DAY_DLYED_15_PERC_PREV_WEEK,
      RWK_DLYED_15_PERC_DIF_PREV_WEEK = RWK_DLYED_15_PERC -
        RWK_DLYED_15_PERC_PREV_WEEK,

      # dif prev year
      ##traffic
      DAY_TFC_DIF_PREV_YEAR = coalesce(DAY_TFC, 0) -
        coalesce(DAY_TFC_PREV_YEAR, 0),
      DAY_TFC_DIF_PREV_YEAR_PERC = if_else(
        DAY_TFC_PREV_YEAR == 0,
        NA,
        DAY_TFC / DAY_TFC_PREV_YEAR
      ) -
        1,
      RWK_TFC_DIF_PREV_YEAR_PERC = if_else(
        RWK_AVG_TFC_PREV_YEAR == 0,
        NA,
        RWK_AVG_TFC / RWK_AVG_TFC_PREV_YEAR
      ) -
        1,
      Y2D_TFC_DIF_PREV_YEAR_PERC = if_else(
        Y2D_AVG_TFC_PREV_YEAR == 0,
        NA,
        Y2D_AVG_TFC / Y2D_AVG_TFC_PREV_YEAR
      ) -
        1,
      S2D_TFC_DIF_PREV_YEAR_PERC = if_else(
        S2D_AVG_TFC_PREV_YEAR == 0,
        NA,
        S2D_AVG_TFC / S2D_AVG_TFC_PREV_YEAR
      ) -
        1,
      ...
      DAY_DLY_DIF_PREV_YEAR_PERC = if_else(
        DAY_DLY_PREV_YEAR == 0,
        NA,
        DAY_DLY / DAY_DLY_PREV_YEAR
      ) -
        1,
      RWK_DLY_DIF_PREV_YEAR_PERC = if_else(
        RWK_AVG_DLY_PREV_YEAR == 0,
        NA,
        RWK_AVG_DLY / RWK_AVG_DLY_PREV_YEAR
      ) -
        1,
      Y2D_DLY_DIF_PREV_YEAR_PERC = if_else(
        Y2D_AVG_DLY_PREV_YEAR == 0,
        NA,
        Y2D_AVG_DLY / Y2D_AVG_DLY_PREV_YEAR
      ) -
        1,
      S2D_DLY_DIF_PREV_YEAR_PERC = if_else(
        S2D_AVG_DLY_PREV_YEAR == 0,
        NA,
        S2D_AVG_DLY / S2D_AVG_DLY_PREV_YEAR
      ) -
        1,

      ##delay ert
      DAY_DLY_ERT_DIF_PREV_YEAR_PERC = if_else(
        DAY_DLY_ERT_PREV_YEAR == 0,
        NA,
        DAY_DLY_ERT / DAY_DLY_ERT_PREV_YEAR
      ) -
        1,
      RWK_DLY_ERT_DIF_PREV_YEAR_PERC = if_else(
        RWK_AVG_DLY_ERT_PREV_YEAR == 0,
        NA,
        RWK_AVG_DLY_ERT / RWK_AVG_DLY_ERT_PREV_YEAR
      ) -
        1,
      Y2D_DLY_ERT_DIF_PREV_YEAR_PERC = if_else(
        Y2D_AVG_DLY_ERT_PREV_YEAR == 0,
        NA,
        Y2D_AVG_DLY_ERT / Y2D_AVG_DLY_ERT_PREV_YEAR
      ) -
        1,
      S2D_DLY_ERT_DIF_PREV_YEAR_PERC = if_else(
        S2D_AVG_DLY_ERT_PREV_YEAR == 0,
        NA,
        S2D_AVG_DLY_ERT / S2D_AVG_DLY_ERT_PREV_YEAR
      ) -
        1,

      ##delay arp
      DAY_DLY_ARP_DIF_PREV_YEAR_PERC = if_else(
        DAY_DLY_ARP_PREV_YEAR == 0,
        NA,
        DAY_DLY_ARP / DAY_DLY_ARP_PREV_YEAR
      ) -
        1,
      RWK_DLY_ARP_DIF_PREV_YEAR_PERC = if_else(
        RWK_AVG_DLY_ARP_PREV_YEAR == 0,
        NA,
        RWK_AVG_DLY_ARP / RWK_AVG_DLY_ARP_PREV_YEAR
      ) -
        1,
      Y2D_DLY_ARP_DIF_PREV_YEAR_PERC = if_else(
        Y2D_AVG_DLY_ARP_PREV_YEAR == 0,
        NA,
        Y2D_AVG_DLY_ARP / Y2D_AVG_DLY_ARP_PREV_YEAR
      ) -
        1,
      S2D_DLY_ARP_DIF_PREV_YEAR_PERC = if_else(
        S2D_AVG_DLY_ARP_PREV_YEAR == 0,
        NA,
        S2D_AVG_DLY_ARP / S2D_AVG_DLY_ARP_PREV_YEAR
      ) -
        1,

      ##delay/flight
      DAY_DLY_FLT_DIF_PREV_YEAR_PERC = if_else(
        DAY_DLY_FLT_PREV_YEAR == 0,
        NA,
        DAY_DLY_FLT / DAY_DLY_FLT_PREV_YEAR
      ) -
        1,
      RWK_DLY_FLT_DIF_PREV_YEAR_PERC = if_else(
        RWK_DLY_FLT_PREV_YEAR == 0,
        NA,
        RWK_DLY_FLT / RWK_DLY_FLT_PREV_YEAR
      ) -
        1,
      Y2D_DLY_FLT_DIF_PREV_YEAR_PERC = if_else(
        Y2D_DLY_FLT_PREV_YEAR == 0,
        NA,
        Y2D_DLY_FLT / Y2D_DLY_FLT_PREV_YEAR
      ) -
        1,
      S2D_DLY_FLT_DIF_PREV_YEAR_PERC = if_else(
        S2D_DLY_FLT_PREV_YEAR == 0,
        NA,
        S2D_DLY_FLT / S2D_DLY_FLT_PREV_YEAR
      ) -
        1,

      ##delay/flight ert
      DAY_DLY_ERT_FLT_DIF_PREV_YEAR_PERC = if_else(
        DAY_DLY_ERT_FLT_PREV_YEAR == 0,
        NA,
        DAY_DLY_ERT_FLT / DAY_DLY_ERT_FLT_PREV_YEAR
      ) -
        1,
      RWK_DLY_ERT_FLT_DIF_PREV_YEAR_PERC = if_else(
        RWK_DLY_ERT_FLT_PREV_YEAR == 0,
        NA,
        RWK_DLY_ERT_FLT / RWK_DLY_ERT_FLT_PREV_YEAR
      ) -
        1,
      Y2D_DLY_ERT_FLT_DIF_PREV_YEAR_PERC = if_else(
        Y2D_DLY_ERT_FLT_PREV_YEAR == 0,
        NA,
        Y2D_DLY_ERT_FLT / Y2D_DLY_ERT_FLT_PREV_YEAR
      ) -
        1,
      S2D_DLY_ERT_FLT_DIF_PREV_YEAR_PERC = if_else(
        S2D_DLY_ERT_FLT_PREV_YEAR == 0,
        NA,
        S2D_DLY_ERT_FLT / S2D_DLY_ERT_FLT_PREV_YEAR
      ) -
        1,

      ##delay/flight arp
      DAY_DLY_ARP_FLT_DIF_PREV_YEAR_PERC = if_else(
        DAY_DLY_ARP_FLT_PREV_YEAR == 0,
        NA,
        DAY_DLY_ARP_FLT / DAY_DLY_ARP_FLT_PREV_YEAR
      ) -
        1,
      RWK_DLY_ARP_FLT_DIF_PREV_YEAR_PERC = if_else(
        RWK_DLY_ARP_FLT_PREV_YEAR == 0,
        NA,
        RWK_DLY_ARP_FLT / RWK_DLY_ARP_FLT_PREV_YEAR
      ) -
        1,
      Y2D_DLY_ARP_FLT_DIF_PREV_YEAR_PERC = if_else(
        Y2D_DLY_ARP_FLT_PREV_YEAR == 0,
        NA,
        Y2D_DLY_ARP_FLT / Y2D_DLY_ARP_FLT_PREV_YEAR
      ) -
        1,
      S2D_DLY_ARP_FLT_DIF_PREV_YEAR_PERC = if_else(
        S2D_DLY_ARP_FLT_PREV_YEAR == 0,
        NA,
        S2D_DLY_ARP_FLT / S2D_DLY_ARP_FLT_PREV_YEAR
      ) -
        1,
      ...
      ##delayed flights 15 min
      DAY_DLYED_15_PERC_DIF_PREV_YEAR = DAY_DLYED_15_PERC -
        DAY_DLYED_15_PERC_PREV_YEAR,
      RWK_DLYED_15_PERC_DIF_PREV_YEAR = RWK_DLYED_15_PERC -
        RWK_DLYED_15_PERC_PREV_YEAR,
      Y2D_DLYED_15_PERC_DIF_PREV_YEAR = Y2D_DLYED_15_PERC -
        Y2D_DLYED_15_PERC_PREV_YEAR,
      S2D_DLYED_15_PERC_DIF_PREV_YEAR = S2D_DLYED_15_PERC -
        S2D_DLYED_15_PERC_PREV_YEAR,

      # dif 2019
      ##traffic
      DAY_TFC_DIF_2019 = coalesce(DAY_TFC, 0) - coalesce(DAY_TFC_2019, 0),
      DAY_TFC_DIF_2019_PERC = if_else(
        DAY_TFC_2019 == 0,
        NA,
        DAY_TFC / DAY_TFC_2019
      ) -
        1,
      RWK_TFC_DIF_2019_PERC = if_else(
        RWK_AVG_TFC_2019 == 0,
        NA,
        RWK_AVG_TFC / RWK_AVG_TFC_2019
      ) -
        1,
      Y2D_TFC_DIF_2019_PERC = if_else(
        Y2D_AVG_TFC_2019 == 0,
        NA,
        Y2D_AVG_TFC / Y2D_AVG_TFC_2019
      ) -
        1,
      S2D_TFC_DIF_2019_PERC = if_else(
        S2D_AVG_TFC_2019 == 0,
        NA,
        S2D_AVG_TFC / S2D_AVG_TFC_2019
      ) -
        1,
      ...
      DAY_DLY_DIF_2019_PERC = if_else(
        DAY_DLY_2019 == 0,
        NA,
        DAY_DLY / DAY_DLY_2019
      ) -
        1,
      RWK_DLY_DIF_2019_PERC = if_else(
        RWK_AVG_DLY_2019 == 0,
        NA,
        RWK_AVG_DLY / RWK_AVG_DLY_2019
      ) -
        1,
      Y2D_DLY_DIF_2019_PERC = if_else(
        Y2D_AVG_DLY_2019 == 0,
        NA,
        Y2D_AVG_DLY / Y2D_AVG_DLY_2019
      ) -
        1,
      S2D_DLY_DIF_2019_PERC = if_else(
        S2D_AVG_DLY_2019 == 0,
        NA,
        S2D_AVG_DLY / S2D_AVG_DLY_2019
      ) -
        1,
      ...
      DAY_DLY_ERT_DIF_2019_PERC = if_else(
        DAY_DLY_ERT_2019 == 0,
        NA,
        DAY_DLY_ERT / DAY_DLY_ERT_2019
      ) -
        1,
      RWK_DLY_ERT_DIF_2019_PERC = if_else(
        RWK_AVG_DLY_ERT_2019 == 0,
        NA,
        RWK_AVG_DLY_ERT / RWK_AVG_DLY_ERT_2019
      ) -
        1,
      Y2D_DLY_ERT_DIF_2019_PERC = if_else(
        Y2D_AVG_DLY_ERT_2019 == 0,
        NA,
        Y2D_AVG_DLY_ERT / Y2D_AVG_DLY_ERT_2019
      ) -
        1,
      S2D_DLY_ERT_DIF_2019_PERC = if_else(
        S2D_AVG_DLY_ERT_2019 == 0,
        NA,
        S2D_AVG_DLY_ERT / S2D_AVG_DLY_ERT_2019
      ) -
        1,

      ##delay arp
      DAY_DLY_ARP_DIF_2019_PERC = if_else(
        DAY_DLY_ARP_2019 == 0,
        NA,
        DAY_DLY_ARP / DAY_DLY_ARP_2019
      ) -
        1,
      RWK_DLY_ARP_DIF_2019_PERC = if_else(
        RWK_AVG_DLY_ARP_2019 == 0,
        NA,
        RWK_AVG_DLY_ARP / RWK_AVG_DLY_ARP_2019
      ) -
        1,
      Y2D_DLY_ARP_DIF_2019_PERC = if_else(
        Y2D_AVG_DLY_ARP_2019 == 0,
        NA,
        Y2D_AVG_DLY_ARP / Y2D_AVG_DLY_ARP_2019
      ) -
        1,
      S2D_DLY_ARP_DIF_2019_PERC = if_else(
        S2D_AVG_DLY_ARP_2019 == 0,
        NA,
        S2D_AVG_DLY_ARP / S2D_AVG_DLY_ARP_2019
      ) -
        1,
      ...
      DAY_DLY_FLT_DIF_2019_PERC = if_else(
        DAY_DLY_FLT_2019 == 0,
        NA,
        DAY_DLY_FLT / DAY_DLY_FLT_2019
      ) -
        1,
      RWK_DLY_FLT_DIF_2019_PERC = if_else(
        RWK_DLY_FLT_2019 == 0,
        NA,
        RWK_DLY_FLT / RWK_DLY_FLT_2019
      ) -
        1,
      Y2D_DLY_FLT_DIF_2019_PERC = if_else(
        Y2D_DLY_FLT_2019 == 0,
        NA,
        Y2D_DLY_FLT / Y2D_DLY_FLT_2019
      ) -
        1,
      S2D_DLY_FLT_DIF_2019_PERC = if_else(
        S2D_DLY_FLT_2019 == 0,
        NA,
        S2D_DLY_FLT / S2D_DLY_FLT_2019
      ) -
        1,

      ##delay/flight ert
      DAY_DLY_ERT_FLT_DIF_2019_PERC = if_else(
        DAY_DLY_ERT_FLT_2019 == 0,
        NA,
        DAY_DLY_ERT_FLT / DAY_DLY_ERT_FLT_2019
      ) -
        1,
      RWK_DLY_ERT_FLT_DIF_2019_PERC = if_else(
        RWK_DLY_ERT_FLT_2019 == 0,
        NA,
        RWK_DLY_ERT_FLT / RWK_DLY_ERT_FLT_2019
      ) -
        1,
      Y2D_DLY_ERT_FLT_DIF_2019_PERC = if_else(
        Y2D_DLY_ERT_FLT_2019 == 0,
        NA,
        Y2D_DLY_ERT_FLT / Y2D_DLY_ERT_FLT_2019
      ) -
        1,
      S2D_DLY_ERT_FLT_DIF_2019_PERC = if_else(
        S2D_DLY_ERT_FLT_2019 == 0,
        NA,
        S2D_DLY_ERT_FLT / S2D_DLY_ERT_FLT_2019
      ) -
        1,

      ##delay/flight arp
      DAY_DLY_ARP_FLT_DIF_2019_PERC = if_else(
        DAY_DLY_ARP_FLT_2019 == 0,
        NA,
        DAY_DLY_ARP_FLT / DAY_DLY_ARP_FLT_2019
      ) -
        1,
      RWK_DLY_ARP_FLT_DIF_2019_PERC = if_else(
        RWK_DLY_ARP_FLT_2019 == 0,
        NA,
        RWK_DLY_ARP_FLT / RWK_DLY_ARP_FLT_2019
      ) -
        1,
      Y2D_DLY_ARP_FLT_DIF_2019_PERC = if_else(
        Y2D_DLY_ARP_FLT_2019 == 0,
        NA,
        Y2D_DLY_ARP_FLT / Y2D_DLY_ARP_FLT_2019
      ) -
        1,
      S2D_DLY_ARP_FLT_DIF_2019_PERC = if_else(
        S2D_DLY_ARP_FLT_2019 == 0,
        NA,
        S2D_DLY_ARP_FLT / S2D_DLY_ARP_FLT_2019
      ) -
        1,
    ...
    mutate(across(
      ...
      ~ if_else(
        (STK_CODE == "IS_ANSP" | substr(STK_CODE, 1, 2) == "BI"),
        NA,
        .x
      )
    )) %>%
    ...
    mutate(across(
      ...
      ~ if_else(
        (STK_CODE == "IS_ANSP" | substr(STK_CODE, 1, 2) == "BI") & YEAR < 2025,
        NA,
        .x
      )
    )) %>%
    ...
    mutate(across(
      ...
      ~ if_else(
        (STK_CODE == "IS_ANSP" | substr(STK_CODE, 1, 2) == "BI") & YEAR < 2024,
        NA,
        .x
      )
    )) %>%
...
stk_aggregate <- function(
  ...
  day_2019 <- mydate -
    days(364 * (year(mydate) - 2019) + floor((year(mydate) - 2019) / 4) * 7)
  current_year = year(mydate)
    ...
    mutate(
      FLAG_PERIOD = case_when(
        ...
        !!(period_type == "W") ~ case_when(
          between(
            DATE_FIELD,
            !!(mydate - lubridate::days(6)),
            !!mydate
          ) ~ "CURRENT_ROLLING_WEEK",
          between(
            DATE_FIELD,
            !!(day_prev_week - lubridate::days(6)),
            !!day_prev_week
          ) ~ "PREV_ROLLING_WEEK",
          between(
            DATE_FIELD,
            !!(day_2019 - lubridate::days(6)),
            !!day_2019
          ) ~ "ROLLING_WEEK_2019",
          between(
            DATE_FIELD,
            !!(day_prev_year - lubridate::days(6)),
            !!day_prev_year
          ) ~ "ROLLING_WEEK_PREV_YEAR",
          TRUE ~ NA_character_
        ),
  ...
  if (agg_stk == 'iso_country') {
    ...
    df_joined <- df %>%
      ...
      mutate(
        AGG_NAME = case_when(
          EC_AP_NAME_1 <= EC_AP_NAME_2 ~ paste0(
            EC_AP_NAME_1,
            "<->",
            EC_AP_NAME_2
          ),
    ...
    df_joined <- df %>%
      ...
      left_join(dim_ao_group1, by = join_by(AGG_ID == AO_GRP_NAME)) %>%
      mutate(AGG_NAME = AGG_ID, AGG_CODE = AO_GRP_CODE) %>%
      filter(TIL >= TO_DATE) %>%
      group_by(.rid) %>%
  ...
  df_period <- df_joined %>%
    ...
    mutate(
      ...
      RANK_PREV_WEEK = case_when(
        FLAG_PERIOD == flag_prev_w &
          !(period_type %in% c("Y", "S")) ~ min_rank(desc(METRIC1)),
        .default = NA_integer_
      ),
  ...
  df_out <- df_period %>%
    ...
    mutate(
      across(
        c(RANK_2019),
        ~ if_else(
          STK_CODE %in% c('IS', 'BIRK', 'BIFK', 'ICE', 'FPY_GRP', "IS_ANSP"),
          NA,
          .x
        )
      ),
      ...
      across(
        ...
        ~ if_else(
          STK_CODE %in%
            c('IS', 'BIRK', 'BIFK', 'ICE', 'FPY_GRP', "IS_ANSP") &
            YEAR_DATA < 2025,
          NA,
          .x
        )
...
run_for_date <- function(day_seq) {
  ...
  df_source <- tryCatch(
    ...
    error = function(e) {
      message(
        "Source partition not found (or unreadable); continuing without df_source."
      )
