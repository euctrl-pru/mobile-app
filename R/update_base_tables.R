...
if (last_run_day == today()) {
  if (floor_date(last_run_time_ok, "day", ) == today()) {
    ...
    msg = paste0(
      "Last time the procedure was run successfully was ",
      last_run_time_ok
    )
  }
} else {
...
check_day <- max(synthesis_last_day$FLIGHT_DATE, na.rm = TRUE) ==
  today() - days(1)
check_flt <- synthesis_last_day %>%
  filter(FLIGHT_DATE == max(FLIGHT_DATE, na.rm = TRUE)) %>%
  select(TOT_TFC) %>%
  pull() !=
  0
...
update_base_tables <- function(mydataframe) {
  ...
  file.copy(
    from = here(base_tables_dir, substr(mydataframe, 1, 2), mydataframe),
    to = dest_parent,
    recursive = TRUE,
    overwrite = TRUE,
    copy.mode = TRUE
  ...
  df <- read_selected_years_duck_tbl(
    con = con,
  ...
  if ("ENTRY_DATE" %in% colnames(df)) {
    df_filtered <- df %>%
      collect() %>%
      filter(
        !(ENTRY_DATE %in% c(seq.Date(today_date - days(days_back), today_date)))
      )
    ...
    df_filtered <- df %>%
      ...
      filter(
        !(FLIGHT_DATE %in%
          c(seq.Date(today_date - days(days_back), today_date)))
      )
  } else if ("ARR_DATE" %in% colnames(df)) {
    df_filtered <- df %>%
      collect() %>%
      filter(
        !(ARR_DATE %in% c(seq.Date(today_date - days(days_back), today_date)))
      )
  }
