...
knit: (function(input, ...) {
    rmarkdown::render(
      input,
      output_dir = "C:/Users/oaolive/repos/mobile-app/iframes"
    )
  })
---
...
apt_rank_data_raw <-  read_xlsx(
  ...
  range = cell_limits(c(19, 2), c(NA, 25))) %>%
  as_tibble()

current_year <- as.numeric(format(max(apt_rank_data_raw$ENTRY_DATE), "%Y"))
...
apt_rank_data_day <- apt_rank_data_raw %>%
  filter(R_RANK_BY_DAY < 11) %>% 
  mutate(RANKDIF = R_RANK_BY_DAY- R_RANK_BY_DAY_7DAY) %>%
  select(RANKDIF,
         AIRPORT_NAME,
         DEP_ARR,
...
ao_rank_day_tbl <- reactable(
  ...
  columns = list(
    ...
    AIRPORT_NAME = colDef(name = "AIRPORT", minWidth = 32),
    DEP_ARR = colDef(name = "FLIGHTS", minWidth = 30,
      style = function(value) {
        bar_style(width = value / max(apt_rank_data_day$DEP_ARR), fill = apt_color, color = "white")
      },
      align = "left",
      format = colFormat(digits = 0, separators = TRUE)
    ),
    DEP_ARR_7DAY_DIFF_PERC = colDef(name = "% vs PREV WEEK",
                                   minWidth = 12,
                                   cell = function(value) {
                                     label <-paste0(if_else(value>=0,'+','-'),format(round(abs(value)*100,0),nsmall=0),'%')
                                   }
                                   ),
    DEP_ARR_PREV_YEAR_DIFF_PERC = colDef(name = paste0("% vs ", current_year-1),
                              minWidth = 12,
                              cell = function(value) {
                                label <-paste0(if_else(value>=0,'+','-'),format(round(abs(value)*100,0),nsmall=0),'%')
                              }
