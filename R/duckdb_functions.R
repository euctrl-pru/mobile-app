...
read_selected_years_duck_tbl <- function(
  con = con,
  mydataframe,
...
save_partitions_from_local_df <- function(
  ...
  on.exit(
    try(duckdb::duckdb_unregister(con, rel_name), silent = TRUE),
    add = TRUE
  )
...
read_partitioned_parquet_duckdb <- function(
  con,
  mydataframe,
  years = NULL,
  subpattern = NULL,
