...
ap_traffic_delay_data <- read_parquet(here(
  app_tables_dir,
  stakeholder,
  mydatafile
)) %>%
...
apt_traffic_for_json <- ap_traffic_delay_data_last_day %>%
  right_join(
    list_airport_extended,
    by = c("ARP_CODE" = "EC_AP_CODE", "ARP_NAME" = "EC_AP_NAME")
  ) %>%
...
apt_delay_for_json <- ap_traffic_delay_data_last_day %>%
  ...
  mutate(
    ...
    Y2D_DLY_RANK = if_else(
      FLAG_TOP_APT == "N",
      NA,
      rank(desc(Y2D_DLY), ties.method = "max")
    ),
    S2D_DLY_RANK = if_else(
      FLAG_TOP_APT == "N",
      NA,
  ...
  arrange(ARP_CODE, ARP_NAME)


#### Punctuality data ----
#querying the data in SQL
apt_punct_raw <- export_query(query_ap_punct(format(
  data_day_date,
  "%Y-%m-%d"
))) %>%
  as_tibble() %>%
...
last_day_punct <- min(max(apt_punct_raw$DAY_DATE), data_day_date, na.rm = TRUE)
...
apt_punct_for_json <- merge(
  apt_punct_d_w,
  apt_punct_y2d,
  by = c("ARP_CODE", "ARP_NAME")
) %>%
  ...
  mutate(
    DY_ARR_PUN_RANK = if_else(
      FLAG_TOP_APT == "N",
      NA,
      min_rank(desc(DY_ARR_PUN))
    ),
    WK_ARR_PUN_RANK = if_else(
      FLAG_TOP_APT == "N",
      NA,
      min_rank(desc(WK_ARR_PUN))
    ),
    Y2D_ARR_PUN_RANK = if_else(
      FLAG_TOP_APT == "N",
      NA,
      min_rank(desc(Y2D_ARR_PUN))
    ),
    S2D_ARR_PUN_RANK = if_else(
      FLAG_TOP_APT == "N",
      NA,
      min_rank(desc(S2D_ARR_PUN))
    ),

    DY_DEP_PUN_RANK = if_else(
      FLAG_TOP_APT == "N",
      NA,
      min_rank(desc(DY_DEP_PUN))
    ),
    WK_DEP_PUN_RANK = if_else(
      FLAG_TOP_APT == "N",
      NA,
      min_rank(desc(WK_DEP_PUN))
    ),
    Y2D_DEP_PUN_RANK = if_else(
      FLAG_TOP_APT == "N",
      NA,
      min_rank(desc(Y2D_DEP_PUN))
    ),
    S2D_DEP_PUN_RANK = if_else(
      FLAG_TOP_APT == "N",
      NA,
      min_rank(desc(S2D_DEP_PUN))
    ),
...
apt_json_app_j <- list_airport_extended %>%
  ...
  arrange(apt_name)
apt_json_app_j$apt_traffic <- select(
  arrange(apt_traffic_for_json, ARP_NAME),
  -c(ARP_NAME, ARP_CODE)
)
apt_json_app_j$apt_delay <- select(
  arrange(apt_delay_for_json, ARP_NAME),
  -c(ARP_NAME, ARP_CODE)
)
apt_json_app_j$apt_punct <- select(
  arrange(apt_punct_for_json, ARP_NAME),
  -c(ARP_NAME, ARP_CODE)
)
...
apt_ao_ranking_traffic <- apt_ao_main_traffic |>
  left_join(
    apt_ao_main_traffic_dif,
    by = c("RANK", "APT_CODE", "APT_NAME"),
...
save_json(
  apt_ao_ranking_traffic_j,
  "apt_ao_ranking_traffic",
  archive_file = FALSE
)
...
apt_apt_ranking_traffic <- apt_apt_main_traffic |>
  ...
  arrange(APT_CODE, APT_NAME, RANK) |>
  distinct(APT_CODE, APT_NAME, RANK, .keep_all = TRUE)

# convert to json and save in app data folder
apt_apt_ranking_traffic_j <- apt_apt_ranking_traffic |> toJSON(pretty = TRUE)

save_json(
  apt_apt_ranking_traffic_j,
  "apt_apt_ranking_traffic",
  archive_file = FALSE
)
...
save_json(
  apt_st_ranking_traffic_j,
  "apt_st_ranking_traffic",
  archive_file = FALSE
)
...
ms_ap_full <- crossing(list_marktet_segment_app, list_airport$EC_AP_CODE) %>%
  select(CODE = MS_ID, NAME = MS_NAME, STK_CODE = contains("EC_AP_CODE")) %>%
  arrange(STK_CODE, CODE)
...
apt_ms_data_day_int <- create_ranking(mydataframe, "DAY", DEP_ARR) %>%
  ...
  group_by(STK_CODE) |>
  mutate(
    SHARE = ifelse(CURRENT == 0, 0, CURRENT / sum(CURRENT, na.rm = TRUE))
  ) |>
  ungroup()
...
save_json(
  apt_ms_ranking_traffic_j,
  "apt_ms_ranking_traffic",
  archive_file = FALSE
)
...
apt_traffic_evo_long <- apt_traffic_evo %>%
  pivot_longer(
    -c(APT_CODE, APT_NAME, FLIGHT_DATE),
    names_to = 'year',
    values_to = 'RWK_AVG_TFC'
  ) %>%
...
apt_punct_evo <- apt_punct_raw %>%
  ...
  right_join(
    select(list_airport_extended, EC_AP_CODE, EC_AP_NAME),
    join_by("ARP_CODE" == "EC_AP_CODE")
  ) %>%
  select(
    ARP_CODE,
    ARP_NAME,
...
apt_delay_cause_day <- ap_traffic_delay_data %>%
  filter(FLIGHT_DATE == min(max(FLIGHT_DATE), data_day_date, na.rm = TRUE)) %>%
  mutate(
...
apt_delay_cause_day_long <- cbind(
  apt_delay_value_day_long,
  apt_delay_share_day_long
) %>%
  select(-name) %>%
  group_by(APT_CODE, APT_NAME, FLIGHT_DATE) %>%
  nest_legacy(.key = "statistics")
...
apt_delay_cause_evo_dy_j <- apt_delay_cause_day_long %>%
  toJSON(., pretty = TRUE)
...
apt_delay_cause_wk <- ap_traffic_delay_data %>%
  filter(
    FLIGHT_DATE >=
      min(max(FLIGHT_DATE), data_day_date, na.rm = TRUE) + lubridate::days(-6),
    FLIGHT_DATE <= min(max(FLIGHT_DATE), data_day_date, na.rm = TRUE)
  ) %>%
  group_by(ARP_CODE) %>%
  mutate(
    WK_SHARE_DLY_G = if_else(sum(DY_DLY) == 0, 0, sum(DY_DLY_G) / sum(DY_DLY)),
    WK_SHARE_DLY_CS = if_else(
      sum(DY_DLY) == 0,
      0,
      sum(DY_DLY_CS) / sum(DY_DLY)
    ),
    WK_SHARE_DLY_IT = if_else(
      sum(DY_DLY) == 0,
      0,
      sum(DY_DLY_IT) / sum(DY_DLY)
    ),
    WK_SHARE_DLY_WD = if_else(
      sum(DY_DLY) == 0,
      0,
      sum(DY_DLY_WD) / sum(DY_DLY)
    ),
    WK_SHARE_DLY_OTHER = if_else(
      sum(DY_DLY) == 0,
      0,
      sum(DY_DLY_OTHER) / sum(DY_DLY)
    )
  ) %>%
  ungroup() %>%
  select(
...
apt_delay_cause_wk_long <- cbind(
  apt_delay_value_wk_long,
  apt_delay_share_wk_long
) %>%
  select(-name) %>%
  group_by(APT_CODE, APT_NAME, FLIGHT_DATE) %>%
  nest_legacy(.key = "statistics")
...
apt_delay_cause_y2d <- ap_traffic_delay_data %>%
  ...
  mutate(
    ...
    Y2D_SHARE_DLY_CS = if_else(
      sum(DY_DLY) == 0,
      0,
      sum(DY_DLY_CS) / sum(DY_DLY)
    ),
    Y2D_SHARE_DLY_IT = if_else(
      sum(DY_DLY) == 0,
      0,
      sum(DY_DLY_IT) / sum(DY_DLY)
    ),
    Y2D_SHARE_DLY_WD = if_else(
      sum(DY_DLY) == 0,
      0,
      sum(DY_DLY_WD) / sum(DY_DLY)
    ),
    Y2D_SHARE_DLY_OTHER = if_else(
      sum(DY_DLY) == 0,
      0,
      sum(DY_DLY_OTHER) / sum(DY_DLY)
    )
...
apt_delay_cause_y2d_long <- cbind(
  apt_delay_value_y2d_long,
  apt_delay_share_y2d_long
) %>%
  select(-name) %>%
  group_by(APT_CODE, APT_NAME, FLIGHT_DATE) %>%
  nest_legacy(.key = "statistics")
...
# for consistency with v1 we use the word category in the name files... should have been cause
apt_delay_cause_evo_y2d_j <- apt_delay_cause_y2d_long %>%
  toJSON(., pretty = TRUE)
...
apt_delayed_flights_evo <- ap_traffic_delay_data %>%
  ...
  mutate(
    ...
    RWK_DELAYED_TFC_PERC_PREV_YEAR = if_else(
      ARP_CODE == "ESSB" &
        FLIGHT_DATE %in% seq.Date(ymd(20250727), ymd(20250803)),
      NA,
      RWK_DELAYED_TFC_PERC_PREV_YEAR
    ),
    RWK_DELAYED_TFC_15_PERC_PREV_YEAR = if_else(
      ARP_CODE == "ESSB" &
        FLIGHT_DATE %in% seq.Date(ymd(20250727), ymd(20250803)),
      NA,
...
apt_delayed_flights_evo_j <- apt_delayed_flights_evo_long %>%
  toJSON(., pretty = TRUE)
